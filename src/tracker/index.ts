...
import { HoldingRecord, MarketData } from "../types";
    ...
    if (holdings.length !== 0) {
      ...
      await rpcManager.withConnection(async (connection) => {
        ...
        for (const row of updatedHoldings) {
          ...
          const tokenList = tokenAddresses as string[];
          const cachedPrices = tokenList.map(token => {
            const priceData = priceCache.get(token);
            if (!priceData || priceData.price <= 0) {
              priceCache.invalidate(token); // Use invalidate instead of delete
              return null;
            }
            return { token, price: priceData.price };
          }).filter((item): item is { token: string; price: number } => item !== null);
            ...
            if (token !== 'Eu8hEhTf2MrKmUbxMD1PCdCpKbWKFBdYwCmsoaS3VGCX') {
              ...
              if (unrealizedPnLPercentage >= takeProfitThreshold || unrealizedPnLPercentage <= stopLossThreshold) {
                try {
                  ...
                  if (typeof txResult === "string" && txResult === "TOKEN_ALREADY_SOLD") {
                    shouldLog = false; // Don't include this token in logs
                  } else if (txResult) {
                    sltpMessage = unrealizedPnLPercentage >= takeProfitThreshold ?
                      "Took Profit: " + txResult.signatures[0] :
                      "Stop Loss triggered: " + txResult.signatures[0];
                  }
                } catch (error) {
